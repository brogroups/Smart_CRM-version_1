    ...
    <transition class="slide-modal">
        <div class="modal" v-if="open">
            <div class="modal-content">
                <h2>Купить более одного продукта</h2>
...
export default {
    ...
    methods: {
        async allSales() {
            try {
                const res = await ipcRenderer.invoke(eventNames.listSalesEvent, {});
                if (res.status === 200) {
                    console.log(res);
        ...
        updateProductPrice(index, productId) {
            const selectedProduct = this.allProduct.find(
            ...
            if (selectedProduct) {
                ...
                console.error(`Product with ID ${productId} not found in allProduct`);
                this.products[index] = {
                    ...this.products[index],
                    id: "",
                    price: 0,
        ...
        async handleSubmit() {
            ...
            if (validProducts.length === 0) {
                this.errorMessage =
                    "Iltimos, kamida bitta mahsulot tanlang va uning qiymatlarini to'ldiring!";
            ...
            try {
                const res = await ipcRenderer.invoke( eventNames.createManySalesEvent, JSON.parse(JSON.stringify(payload))  
                );
                if (res) {
                    this.$emit("status", {
                        status: 200,
